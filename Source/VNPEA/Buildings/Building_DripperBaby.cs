...
namespace VNPEA
{
    public class Building_DripperBaby : Building_Dripper
    {
            ...
            for (int i = 0; i < linkeds.Count; i++)
            {
                ...
                if (linked is Building_Bed bed)
                {
                    ...
                    for (int o = 0; o < occupants.Count; o++)
                    {
                        ...
                        {
                            if (occupant.BodySize <= 0.725)
                            {
                                if (occupant.needs.food.CurLevelPercentage <= 0.4)
                                {
                                    net.DrawAmongStorage(1f / 18f, net.storages);
                            ...
                            if (occupant.BodySize > 0.725)
                            {
                                if (occupant.needs.food.CurLevelPercentage <= 0.8)
                                {
                                    net.DrawAmongStorage(1f / 18f, net.storages);
                                    var meal = ThingMaker.MakeThing(ThingDefOf.BabyFood);
                                    if (meal.TryGetComp<CompIngredients>() is CompIngredients ingredients)
